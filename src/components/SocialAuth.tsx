...
const SocialAuth = ({ next, onError }: Props) => {
  ...
  const start = async (provider: "google" | "apple") => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider,
      options: {
        redirectTo: `${window.location.origin}/account?next=${encodeURIComponent(next)}`,
  ...
  return (
    <div className="space-y-3">
      ...
      <button type="button" onClick={() => start("google")} disabled={busy !== null} className={cls}>
      ...
      <button type="button" onClick={() => start("apple")} disabled={busy !== null} className={cls}>
